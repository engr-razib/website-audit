...
export default function ImageDownloaderPage() {
  ...
  // Sync mode with visual picker iframe
  useEffect(() => {
    if (isPickerOpen && iframeRef.current?.contentWindow) {
      iframeRef.current.contentWindow.postMessage({
        type: 'SET_PICKER_MODE',
        active: isPickingActive
      }, '*');
    }
  }, [isPickingActive, isPickerOpen]);
  ...
  useEffect(() => {
    const handler = (e: MessageEvent) => {
      if (e.data?.type === 'SELECTOR_PICKED' && isPickerOpen && isPickingActive) {
            ...
            <div className="w-full md:w-80 bg-slate-900 border-t md:border-t-0 border-slate-800 flex flex-col h-[300px] md:h-full shrink-0">
              <div className="p-4 border-b border-slate-850 shrink-0">
                ...
                <p className="text-[10px] text-slate-400 mt-1 leading-normal">
                  Click any element to map its selector, or switch to <strong>Browse Mode</strong> to explore the page.
                </p>
              </div>
              ...
              <div className="p-3 bg-slate-950/80 border-b border-slate-850 shrink-0 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  {isPickingActive ? (
                    <button
                      type="button"
                      onClick={() => setIsPickingActive(false)}
                    ...
                    >
                      ...
                      Mapping Active (Click to Deselect)
                    </button>
                  ) : (
                ...
                <button
                  type="button"
                  onClick={() => setIsPickingActive(!isPickingActive)}
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <div className={`p-1.5 rounded-lg ${!isPickingActive ? "bg-emerald-500/20 text-emerald-400" : "bg-slate-800 text-slate-400"}`}>
                      <Compass className="h-3.5 w-3.5" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-white truncate">Browse / Data Explore Mode</p>
                      <p className="text-[9px] text-slate-400 truncate">Deselect picker & click page tabs/links</p>
                    </div>
                  </div>
                  {!isPickingActive && (
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                <div
                  onClick={() => setIsPickingActive(true)}
                ...
                >
                  ...
                  {selector ? (
                    <div className="text-[10px] font-mono text-indigo-300 dark:text-indigo-400 break-all bg-slate-950 p-2 rounded-lg border border-slate-855">
                      {selector}
                    </div>
                  ) : (
                    <div className="text-[10px] text-slate-500 italic bg-slate-950/20 p-2 rounded border border-dashed border-slate-850">
                      {isPickingActive ? "Click element on page to map" : "Click here to activate element mapping"}
                    </div>
                  )}
                </div>
              </div>
