                  ...
                  <tr key={idx} className="hover:bg-slate-50/80 dark:hover:bg-slate-900/40 transition-colors">
                    ...
                    <td className="p-3">
                      <div className="space-y-1">
                        <span className="font-mono text-[10px] text-slate-900 dark:text-white bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-1 py-0.5 rounded">
                          &lt;{m.tagName || "div"}&gt;
                        </span>
                        <div className="font-mono text-[10px] text-slate-500 dark:text-slate-400 truncate max-w-[150px]" title={m.selector}>
                          {m.selector}
                        </div>
                      </div>
                    </td>
                    <td className="p-3 max-w-[180px] truncate font-medium text-slate-900 dark:text-slate-200" title={m.textSnippet}>
                      {m.textSnippet || ""}
                    </td>
                    <td className="p-3">
                      <div className="max-h-16 overflow-y-auto text-[10px] font-mono p-1.5 bg-slate-900 dark:bg-slate-950 rounded border border-slate-800 text-emerald-400 w-64 max-w-xs whitespace-pre-wrap scrollbar-thin">
                        {m.outerHTML}
                      </div>
                    </td>
                    <td className="p-3">
                      <div className="max-h-16 overflow-y-auto text-[10px] font-mono p-1.5 bg-slate-900 dark:bg-slate-950 rounded border border-slate-800 text-blue-400 w-64 max-w-xs whitespace-pre-wrap scrollbar-thin">
                        {m.cssStyles}
                      </div>
                    </td>
                    {jobId && (
                      <td className="p-3 text-center">
                        {m.screenshotName && m.screenshotName !== "N/A" && m.screenshotName !== "" ? (
                          <button
                            ...
                            className="inline-flex items-center gap-1 text-[10px] font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 hover:underline cursor-pointer bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 px-2 py-1 rounded border border-indigo-200 dark:border-indigo-500/25 transition-all"
                          >
